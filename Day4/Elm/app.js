...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$Native = Elm.Native.Native.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var _op = {};
   var checkHash = F3(function (secret,start,requirement) {
      checkHash: while (true) {
         var length = $String.length(requirement);
         var test = start + 1;
         var input = A2($Basics._op["++"],secret,$Basics.toString(test));
         var hash = $Native$Native.md5(input);
         if (_U.eq(A2($String.left,length,hash),requirement)) return test; else {
               var _v0 = secret,_v1 = test,_v2 = requirement;
               secret = _v0;
               start = _v1;
               requirement = _v2;
               continue checkHash;
   ...
   var day2part1 = function (input) {    return A3(checkHash,input,-1,"00000");};
   var day2part2 = function (input) {    return A3(checkHash,input,-1,"000000");};
   var view = function (input) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Day 4 Part 1: ",$Basics.toString(day2part1(input))))]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Day 4 Part 2: ",$Basics.toString(day2part2(input))))]))]));
   };
   var main = view("ckczppom");
   return _elm.Main.values = {_op: _op,main: main,view: view,checkHash: checkHash,day2part1: day2part1,day2part2: day2part2};
};
